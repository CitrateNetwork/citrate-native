...
//! defaults to `http://127.0.0.1:8599`. The node must be an anvil with
//! unlocked default accounts (#1 provider, #2 requester) and `anvil_mine`.
...
async fn send(url: &str, from: &str, to: &str, data: &[u8], value: u128) -> Result<(), String> {
    ...
    for _ in 0..100 {
        ...
        if let Some(s) = r.get("status").and_then(Value::as_str) {
            return if s == "0x1" { Ok(()) } else { Err(format!("status {s}")) };
        }
...
async fn call_u128(url: &str, to: &str, data: Vec<u8>) -> u128 {
    let out = rpc(url, "eth_call", json!([{"to": to, "data": format!("0x{}", hex::encode(data))}, "latest"]))
        .await
        .expect("eth_call");
...
async fn settlement_reads_match_the_deployed_contracts() {
    ...
    let url = std::env::var("CITRATE_R2_ANVIL_RPC").unwrap_or_else(|_| "http://127.0.0.1:8599".into());
    let book: Value = serde_json::from_str(&std::fs::read_to_string(path).expect("addrs")).expect("json");
    let market = book["ComputeMarketplace"].as_str().expect("market").to_string();
    let verifier = book["ComputeVerifier"].as_str().expect("verifier").to_string();
    ...
    send(&url, REQUESTER, &market, &post_job(model, &keccak(&[b"in"]), one, 0), one)
        .await
        .expect("postJob");
    ...
    send(&url, REQUESTER, &market, &job_call("assignBestBid(uint256)", job), 0)
        .await
        .expect("assign");
    send(&url, PROVIDER, &market, &job_call("startExecution(uint256)", job), 0)
        .await
        .expect("start");
    ...
    send(&url, PROVIDER, &market, &commit, 0).await.expect("commit");

    let head = reader.head().await.expect("head");
    assert_eq!(reader.commitment_block(job).await.expect("commitmentBlock"), head);
    ...
    send(&url, PROVIDER, &market, &reveal, 0).await.expect("submitResult");

    let verified_at = reader.result_verified_at(job).await.expect("verifiedAt");
    assert_eq!(verified_at, reader.head().await.expect("head"));
    assert!(!reader.dispute_resolved_for_provider(job).await.expect("resolved"));
    // The contract agrees with the relay's window: completeJob reverts
    // before verifiedAt + DISPUTE_WINDOW and succeeds at it.
    assert!(send(&url, PROVIDER, &market, &job_call("completeJob(uint256)", job), 0).await.is_err());
    let now = reader.head().await.expect("head");
    let to_mine = verified_at + DISPUTE_WINDOW_BLOCKS - now - 1;
    rpc(&url, "anvil_mine", json!([format!("0x{to_mine:x}")])).await.expect("mine");
    send(&url, PROVIDER, &market, &job_call("completeJob(uint256)", job), 0)
        .await
    ...
    zk_input[31] = 1;
    send(&url, REQUESTER, &market, &post_job(zk_model, &zk_input, 11 * one, 0), 11 * one)
        .await
    ...
    let bogus = RpcSettlementReader::new(url.clone(), "0x0000000000000000000000000000000000000001", verifier);
