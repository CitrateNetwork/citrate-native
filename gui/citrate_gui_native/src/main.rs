...
fn main() {
    ...
    ui.on_models_deploy_model(move || {
        tracing::info!("Models: deploy model (requires chain tx — not yet implemented)");
        let _ = (&core, &rt_h);
        // Model deployment requires a signed transaction to the ModelRegistry contract.
        // This will be wired in FINAL-3 when real data sources are connected.
    });
