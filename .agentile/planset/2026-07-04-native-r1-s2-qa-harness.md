...
| **A2** | **boot2 (a federation bootnode) `Transport error: eof` after Noise handshake** — reproduced in both survey sessions; do A6 first to rule out protocol drift | Live smoke connects **4/4** bootnodes and holds ≥10 min, OR written root cause + chain-repo issue filed. **Source:** embedded-node peer logs (`node_service`) + boot2 droplet logs; time-boxed 2 days to the root-cause exit. |
